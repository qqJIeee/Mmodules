...
class mlimits(loader.Module):
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                "Sum", "",
                lambda: "Поставьте сумму которой вы будете проверять лимит\nЧтобы изменить через lscfg пишите sm",
                validator=loader.validators.String()
            ),
            loader.ConfigValue(
                "dly", 2.0,
                lambda: "Задержка переводов лимитов",
    ...
    async def lautoset(self,message):
        ...
        if len(args) == 2:
            ...
            timee = time[-1]
            if timee in ['1']:
                await self.client.send_message(message.peer_id, f"✅ <b>| Автоматическая установка лимита игроку <code>{chel}</code> раз в <code>{time}</code> секунду <code>{kolvo}</code> раз начата</b>")
            if timee in ['2', '3', '4']:
                await self.client.send_message(message.peer_id, f"✅ <b>| Автоматическая установка лимита игроку <code>{chel}</code> раз в <code>{time}</code> секунды <code>{kolvo}</code> раз начата</b>")
            if timee in ['5', '6', '7', '8', '9', '0']:
                await self.client.send_message(message.peer_id, f"✅ <b>| Автоматическая установка лимита игроку <code>{chel}</code> раз в <code>{time}</code> секунд <code>{kolvo}</code> раз начата </b>")
            time = int(time)
            if self.limitsxx:
                while self.limitsxx:
                    kolvo -= 1 
    ...
    async def lautosett(self,message):
        nick = self.get('qq')
        tt = self.db.get(self.name, "tt", None)
        dly = self.config["dly"]
        limitsf = self.db.get(self.name, "limitsf", None)
        ...
        timee = tt[-1]
        if timee in ['1']:
                await self.client.send_message(message.peer_id, f"✅ <b>| Автоматическая установка лимита игроку <code>{nick}</code> раз в <code>{tt}</code> секунду <code>{kolvo}</code> раз начата</b>")
        if timee in ['2', '3', '4']:
            await self.client.send_message(message.peer_id, f"✅ <b>| Автоматическая установка лимита игроку <code>{nick}</code> раз в <code>{tt}</code> секунды <code>{kolvo}</code> раз начата</b>")
        if timee in ['5', '6', '7', '8', '9', '0']:
            await self.client.send_message(message.peer_id, f"✅ <b>| Автоматическая установка лимита игроку <code>{nick}</code> раз в <code>{tt}</code> секунд <code>{kolvo}</code> раз начата </b>")
        tt = int(tt)
        if self.limitsxx:
            while self.limitsxx:
                kolvo -= 1 
                if kolvo == 0:
                    self.limitsxx = False
                await self.client.send_message("@mine_evo_bot", f"Перевести {nick} {limitp}")
