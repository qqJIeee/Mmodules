...
class mlimits(loader.Module):
        ...
        if len(args) == 2:
            ...
            if limitp == None:
                ...
                if self.limitsx:
                    ...
                    if limits <= 0:
                        await self.client.send_message(message.peer_id, f"<emoji document_id=5332533929020761310>✅</emoji> <b>Все лимиты игроку <code>{player}</code> переведены:</b> <code>{limitsf}</code>")
                    if message.out:
    ...
    async def mstop(self,message):
        ...
        self.limitsx = False
        await self.client.send_message(message.peer_id, "<emoji document_id=5447644880824181073>⚠️</emoji> Вы остановили перевод лимитов")
        await message.delete()
    @loader.command()
